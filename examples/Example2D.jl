...
for it = 1:nt   # Time loop
    ...
    if mod(it,20)==0  # Visualisation
        ...
        p1          =   heatmap(x_km, z_km, T',         aspect_ratio=1, xlims=(x_km[1],x_km[end]), ylims=(z_km[1],z_km[end]),   c=:lajolla, xlabel="Width [km]",ylabel="Depth [km]", title="$(round(time_kyrs, digits=2)) kyrs", dpi=200, fontsize=6, colorbar_title="Temperature")
        p2          =   heatmap(x_km, z_km, Phi_melt',  aspect_ratio=1, xlims=(x_km[1],x_km[end]), ylims=(z_km[1],z_km[end]),   c=:nuuk,    xlabel="Width [km]",             dpi=200, fontsize=6, colorbar_title="Melt Fraction")
