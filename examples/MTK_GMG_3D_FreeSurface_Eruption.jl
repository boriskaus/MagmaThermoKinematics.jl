...
# NOTE: use `PennyShapedSill` (the Sun-1969 penny-crack solution, like
# examples/MTK_GMG_3D_example.jl), NOT `EllipticalIntrusion`, in 3D. As of
# InjectSills 0.2.0 the 3D `EllipticalIntrusion.hostrock_displacement` is grossly
# oversized (≫ the H/2 opening it should produce), which "inflates" the free
# surface by kilometres per injection. PennyShapedSill is the well-behaved 3D
# source (max host-rock displacement ≈ H/2). W,H are diameters (semi-axes W/2,H/2).
